...
|                   | WeatherPy_challenge.csv         | Input Flle: output file from application 1 containing city locations and current weather information.|
...
|                   | WeatherPy_vacation.csv          | Input File: output file from application 2 which contains location, current weather, and hotel information for the cities selected in application 2.|
|                   | WeatherPy_travel_map.png        | Output File: image of the google directional map for the 4 selected cities with the cities connected by the highlighted roads between them.|
...
* WeatherPy_challenge.csv
   * A csv file containing the location and current weather for the cities generated by this application

### Application 2: Vacation_Search.ipynb
#### Input:
* WeatherPy_challenge.csv 
  * The csv file from the Jupyter Notebook application, Weather_Database.ipynb, containing city location and weather information
#### Description:
This application
* Prompts user to enter the weather criteria for the desired travel destination
* Selects all cities whose current weather information meets the users's destination weather criteria
* For each city selected:
  * Finds a nearby hotel in each city
  * If no nearby hotel is found, the city is deleted from the list
* Creates a google map with location markers for each selected city
...
2. WeatherPy_vacation_map.png
    * A image of the google location map with markers for for the cities that meets the user's destination weather criteria
...
3. WeatherPy_vacation_map_zoom.png
    * A zoomed in image of the google location map showing city location markers with their popup information window displayed
