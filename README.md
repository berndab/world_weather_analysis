...
|                   | WeatherPy_vacation_map_zoom.png | Output File: a zoomed image of google location map wich shows a close up of a city location marker which has been clicked and the popup information window for that city marker is displayed|
|  Jupyter Notebook Application 3    | Vacation_Itinerary.ipynb        | Inputs the city information from application 2 and generates a google directional map displaying a sample of 4 cities from source data set that are geographically near each other. Each city is represented by an individual location marker. The city location markers are connected by highlighted roads to use to travel from once city to the other. Also, this application generates a marker layer google map with location markers for each city that are configured to  to display a popup information window about the city when the marker is clicked.|
